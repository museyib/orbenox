...
function createProductCategory(event) {
  const formData = new FormData(event.target);
  const data = Object.fromEntries(formData.entries());
  data.enabled = enabled.value;
  apiRequest('/api/productCategories', 'POST', data).then(response => {
    if (response.code === 200) {
      router.push('/ui/productCategories');
    } else if (response.code === 401) {
      refreshToken(() => createProductCategory(event), () => router.push('/ui/login'));
    } else {
...
<template>
  <MainLayout>
    <PageHeader :title='$t("productCategory.new")'/>
