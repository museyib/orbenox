...
function createCountry(event) {
  ...
  apiRequest('/api/countries', 'POST', data).then(response => {
    if (response.code === 200) {
      router.push('/ui/countries');
    } else if (response.code === 401) {
      refreshToken(() => createCountry(event), () => router.push('/ui/login'));
    } else {
...
<template>
  <MainLayout>
    <PageHeader :title='$t("country.new")'/>
